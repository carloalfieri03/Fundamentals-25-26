...
def get_dataset_stats(X_train):
    ...
    Assumes X_train shape: (N_Samples, Channels, Time) -> (N, 9, 128)
    """
    # We average over Dimension 0 (Samples) and Dimension 2 (Time)
    # We keep Dimension 1 (Channels) distinct.
...
def apply_normalization(X, mean, std):
    """
    Applies the calculated global stats to any dataset (Train, Val, or Test).
    """
    return (X - mean) / (std + 1e-8)

## Data should be splitted by subject and not randomly otherwise the model may recognize in train and validation the same subjects 

def split_by_subject(dataset, val_split=0.3, seed=42):
    
        # n_splits=1 because we just want one Train/Val pair
        splits = GroupShuffleSplit(n_splits=1, test_size=val_split, random_state=seed)

        # 2. Get the indices
        # We pass 'dataset.subjects' as the 'groups' parameter.
        # The splitter ensures that groups are kept whole.
        ...
        print(f" - Train Samples: {len(train_idx)} (Indices example: {train_idx[:5]})")
        print(f" - Val Samples:   {len(val_idx)}   (Indices example: {val_idx[:5]})")
...
class HarDataset(Dataset):
    ...
    def load_subjects(self):
        # Reads the single column file (e.g., 1, 1, 1, 2, 2...)
...
def load_har(data_dir: Path, batch_size=32, val_split=0.2, num_workers=4):
        full_train_dataset = HarDataset(data_dir, split='train', transform=None)

        train_dataset, val_dataset = split_by_subject(full_train_dataset, val_split=val_split)

        test_dataset = HarDataset(data_dir, split='test', transform=None)
